...
### Cleanup
- [~] Replace `export` → `pub` across all lib/*.crl files (`_visibility` accepts both, `pub` is canonical) — only `wallvm/wallvm.crl` and `wallvm/target/x86_64/x86_64.crl` still use `export`
- [x] Check `static` in more contexts if lib exercises them — heavily used: `static const` (ir_opcodes.crl), `static void` (noret, inline pass), `static bool` (range/gvn passes), `static str/u` (string.crl, startup_templates.crl); grammar handles it
...
- [ ] Revisit remaining attr patterns after the fix: attr followed by `pub`/`extern` decls
  (e.g. `lib/std/platform/libc.crl` style) — check corpus sweep for stragglers.
